...
/hook/startup/proc/loadGenderWhitelist()
	if(config.usegenderwhitelist)
		load_genderwhitelist()
	return 1
...
/proc/load_genderwhitelist()
	var/text = file2text("config/genderblacklist.txt")
	if (!text)
		log_misc("Failed to load config/genderwhitelist.txt")
	else
		gender_whitelist = splittext(text, "\n")
...
/proc/is_gender_whitelisted(mob/M, var/datum/gender/gender)
	//They are admin or the whitelist isn't in use
	if(whitelist_overrides(M))
		return 1
	...
	//If we have a loaded file, search it
	if(gender_whitelist)
		for (var/s in gender_whitelist)
			if(!findtext(s,"[M.ckey] - [gender]"))
				return 1
	else
		return 0
