...
/mob/new_player/Topic(href, href_list[])
	...
	if(href_list["ready"])
		...
		if(!is_gender_whitelisted(src, client.prefs.biological_gender))
			alert(src, "You are currently not whitelisted to play [client.prefs.biological_gender].")
			ready = 0
		...
		if(client.prefs.species != "Human" && !check_rights(R_ADMIN, 0)) //VORESTATION EDITS: THE COMMENTED OUT AREAS FROM LINE 154 TO 178
			if (config.usealienwhitelist)
				if(!is_alien_whitelisted(src, client.prefs.species))
					...
					return 0
*/
		if(client.prefs.biological_gender == "MALE") //SANNA'S SHITCONE
			if(!is_gender_whitelisted(src, client.prefs.biological_gender))
				alert(src, "You are currently blacklisted from playing [client.prefs.biological_gender].")
				return 0
...
/mob/new_player/proc/create_character(var/turf/T)
	...
	chosen_gender = client.prefs.biological_gender
	...
	if(chosen_species && use_species_name)
		...
		if(is_alien_whitelisted(chosen_species))
			if(is_gender_whitelisted(chosen_gender))
